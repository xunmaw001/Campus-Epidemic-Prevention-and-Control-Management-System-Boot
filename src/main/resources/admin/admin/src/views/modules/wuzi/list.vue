        ...
        <div v-if="showFlag">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('wuzi','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/xiaoyuanyiqingfangkongguanlixitong/upload/wuziMuBan.xls"
                        >批量导入物资数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('wuzi','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="wuziUploadSuccess"
                                :on-error="wuziUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('wuzi','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "wuzi.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="wuziKucunNumber"
                                      header-align="center"
                                      label="物资库存">
                        <template slot-scope="scope">
                            {{scope.row.wuziKucunNumber}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('wuzi','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('wuzi','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('wuzi','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
                            <!--<el-button v-if="isAuth('wuzi','修改')" type="primary" icon="el-icon-add-location" size="mini" @click="plusWuziKucunNumber(scope.row.id,scope.row.wuziKucunNumber)">增加物资库存</el-button>-->
                            <!--<el-button v-if="isAuth('wuzi','修改')" type="primary" icon="el-icon-delete-location" size="mini" @click="reduceWuziKucunNumber(scope.row.id,scope.row.wuziKucunNumber)">减少物资库存</el-button>-->
...
<!-- 库存 -->
		<el-dialog
                :title="wuziKucunNumberTitle"
                :visible.sync="wuziKucunNumberVisible"
                width="30%">
            <span>操作数量</span><el-input-number v-model="kucunNumber" :min="1" :max="100" label="描述文字"></el-input-number>
            <span slot="footer" class="dialog-footer">
			<el-button @click="wuziKucunNumberVisible = false">取 消</el-button>
			<el-button type="primary" @click="addWuziKucunNumber()">确 定</el-button>
		  </span>
        </el-dialog>
...
<script>
    ...
    //当前表下拉框搜索条件
        ...
        //库存
            ...
            wuziKucunNumber: 0,
            //标题
            wuziKucunNumberTitle: null,
            //模态框状态
            wuziKucunNumberVisible: false,
            };
        },
        created() {
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'wuzi',//当前表表名,
                        sumColum : 'wuzi_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'wuzi_name',//分组字符串字段
                        // types : 'wuzi_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                params['wuziDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            wuziUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "wuzi/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            wuziUploadError(data){
                this.$message.error('上传失败');
            },
            //增加物资库存
			plusWuziKucunNumber(thisId,wuziKucunNumber){
			//清空数据
				//物资库存数据设置为0
				this.kucunNumber = 0
				//id清空
			...
			//数据不为空放值
				...
				//将模态框打开
				this.wuziKucunNumberVisible = true;
				//标题
				this.wuziKucunNumberTitle = '增加物资库存'
			},
			//减少物资库存
			reduceWuziKucunNumber(thisId,wuziKucunNumber){
			//清空数据
				//物资库存数据设置为0
				this.kucunNumber = 0
				//id清空
			...
			//数据不为空放值
				...
				this.wuziKucunNumberVisible = true;
				//标题
				this.wuziKucunNumberTitle = '减少物资库存'
			},
			//提交用户操作物资库存数量
			addWuziKucunNumber(){
				var type = null
				...
				if(this.kucunNumberTypes == "+"){
					type = '增加'
					maxNumber = this.wuziKucunNumber + this.kucunNumber
				}else{
					type = '减少'
					if(this.wuziKucunNumber < this.kucunNumber || this.kucunNumber <= 0){
						this.$message.error('物资库存不足不能减少');
						return false;
					}else{
						maxNumber = this.wuziKucunNumber - this.kucunNumber
					}
				...
				this.wuziKucunNumberVisible = false;
			},	
            //无用
            wuyong(id) {
                let _this = this;
