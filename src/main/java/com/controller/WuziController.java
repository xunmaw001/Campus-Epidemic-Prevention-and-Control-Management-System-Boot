...
public class WuziController {
    ...
    @Autowired
    private DakaService dakaService;//检测信息
    @Autowired
    ...
    @Autowired
    private GonggaoService gonggaoService;//校园公告
    @Autowired
    private NewsService newsService;//疫情新闻
    @Autowired
    ...
    */
    ...
    public R save(@RequestBody WuziEntity wuzi, HttpServletRequest request){
        ...
        Wrapper<WuziEntity> queryWrapper = new EntityWrapper<WuziEntity>()
            ...
            .eq("wuzi_xinghao", wuzi.getWuziXinghao())
            .eq("wuzi_guige", wuzi.getWuziGuige())
            .eq("wuzi_changjia", wuzi.getWuziChangjia())
...
//                            wuziEntity.setWuziXinghao(data.get(0));                    //物资型号 要改的
//                            wuziEntity.setWuziGuige(data.get(0));                    //物资规格 要改的
//                            wuziEntity.setWuziChangjia(data.get(0));                    //物资生产厂家 要改的
...
//                            wuziEntity.setCreateTime(date);//时间
    ...
    */
    ...
    public R gexingtuijian(@RequestParam Map<String, Object> params, HttpServletRequest request){
        ...
        PageUtils pageUtils = wuziCollectionService.queryPage(params1);
        List<WuziCollectionView> collectionViewsList =(List<WuziCollectionView>)pageUtils.getList();
        Map<Integer,Integer> typeMap=new HashMap<>();//购买的类型list
        for(WuziCollectionView collectionView:collectionViewsList){
            Integer wuziTypes = collectionView.getWuziTypes();
